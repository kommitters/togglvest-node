...
const sync = (command) => async (day = moment().date(), month, year = moment().year()) => {
  try {
    ...
    const harvestEntries = timeEntries.map(({ client, project, description, tags: [tag], duration }) => {
      if (!tag) {
        throw new Error(`The time entry [${client}/${project}] [${description}] has no tag`)
      }

      if (duration < 1) {
        throw new Error(`The time entry [${client}/${project}] [${description}] [${duration}] has no a valid duration`)
      }
